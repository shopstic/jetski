...
export async function updateKubeconfig(
  ...
  if (await fsExists(kubeConfigFile)) {
    const backupKubeConfigFile = joinPath(
      kubeDir,
  ...
  log(
    `Merging instance's kubeconfig to`,
    cyan(kubeConfigFile),
  );
  const newKubeConfig = (await captureExec({
    cmd: ["kubectl", "config", "view", "--flatten"],
    env: {
      KUBECONFIG: `${tempKubeConfigFile}:${kubeConfigFile}`,
    },
  })).out;
  ...
  ok(
    "Kubeconfig has been updated. The current context should now be",
    cyan(name),
  );
}

export async function createInstance(instance: InstanceConfig) {
  const { sshDirectoryPath, cpus, memoryGiBs, diskGiBs, image, name } =
    instance;

  await generateSshKeyPairIfNotExists(instance);

  const sshPublicKey = await getSshPublicKey(sshDirectoryPath);
  const cloudInitConfig = createCloudInitConfig({
    sshPublicKey,
    instance,
  });
  const tempDir = await Deno.makeTempDir();
  ...
  const cloudInitLogTailingPromise = (async () => {
    ...
    return await multipassTailCloudInitOutputLog({
      instance,
      abortSignal: cloudInitLogTailingAbort.signal,
    });
  })();
